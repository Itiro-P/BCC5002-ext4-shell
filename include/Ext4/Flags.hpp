...
namespace Ext4::SuperBlockFlags {
    // Estado de integridade e montagem do sistema de arquivos.
    enum SuperBlockState: uint16_t {
    ...
    enum SuperBlockErrorPolicy: uint16_t {
    ...
    enum SuperBlockCreatorOS: uint32_t {
        OS_LINUX = 0,
        OS_HURD = 1,
        OS_MASIX = 2,
        OS_FREEBSD = 3,
    ...
    enum SuperBlockRevisionLevel: uint32_t {
    ...
    enum SuperBlockCompatFeature: uint32_t {
        ...
        COMPAT_ORPHAN_ALLOC  = 0x1000, // Alocação de arquivos órfãos estruturada de forma nativa para melhor rastreamento.
    };

    // Recursos Incompatíveis: O kernel DEVE recusar a montagem caso não entenda qualquer um desses bits ativos.
    enum SuperBlockIncompatFeature: uint32_t {
        ...
        INCOMPAT_FS_RECOVER       = 0x00004, // O sistema exige reprodução do log do journal antes de poder ser montado em modo leitura-escrita.
        INCOMPAT_SEPARATE_JOURNAL = 0x00008, // O volume utiliza um dispositivo físico externo e dedicado para o log do journal.
        INCOMPAT_META_GROUPS      = 0x00010, // Uso de agrupamentos flexíveis de descritores (Meta block groups) para redimensionamento em larga escala.
        ...
        INCOMPAT_LARGE_FILE       = 0x00080, // Permite que o sistema de arquivos manipule arquivos maiores do que 2GB.
    ...
    enum SuperBlockRoCompatFeature: uint32_t {
        ...
        RO_COMPAT_METADATA_CSUM = 0x0400, // Ativa a proteção global por checksum CRC32c em praticamente todos os metadados do sistema.
        RO_COMPAT_REPLICA       = 0x0800, // Suporte experimental para árvores de réplicas de leitura para tolerância a falhas.
        RO_COMPAT_READONLY      = 0x1000, // Força o sistema a se comportar de forma estritamente somente-leitura em nível lógico interno.
        RO_COMPAT_ORPHAN_PRESENT= 0x2000, // Indica de maneira estática a presença de arquivos na lista de órfãos pendentes.
    };

    // Define o algoritmo usado no recurso HTree para indexar e buscar arquivos dentro das pastas.
    enum SuperBlockDefaultHashVersion : uint8_t {
        HASH_LEGACY            = 0x0,
        HASH_HALF_MD4          = 0x1,
        HASH_TEA               = 0x2,
    ...
    enum SuperBlockDefaultMountOpts : uint32_t {
    ...
    enum SuperBlockIndexFlags : uint32_t {
    ...
    enum SuperBlockEncryptionMode : uint8_t {
