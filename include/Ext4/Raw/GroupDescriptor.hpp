...
namespace Ext4::Raw {
    ...
    struct GroupDescriptor {
        ...
        uint32_t    bg_block_bitmap_lo;       // Bitmap de blocos (bits 0–31).
        ...
        uint16_t    bg_free_blocks_count_lo;  // Quantidade de blocos livres (bits 0–15).
        uint16_t    bg_free_inodes_count_lo;  // Quantidade de inodes livres (bits 0–15).
        uint16_t    bg_used_dirs_count_lo;    // Diretórios no grupo (bits 0–15).
        ...
        uint16_t    bg_itable_unused_lo;      // Inodes não inicializados (bits 0–15).
        ...
        uint32_t    bg_block_bitmap_hi;       // Bitmap de blocos (bits 32–63).
        ...
        uint16_t    bg_free_blocks_count_hi;  // Quantidade de blocos livres (bits 16–31).
        uint16_t    bg_free_inodes_count_hi;  // Quantidade de inodes livres (bits 16–31).
        uint16_t    bg_used_dirs_count_hi;    // Diretórios no grupo (bits 16–31).
        uint16_t    bg_itable_unused_hi;      // Inodes não inicializados (bits 16–31).
        uint32_t    bg_exclude_bitmap_hi;     // Bits superiores (32-63) do endereço do bitmap de exclusão.
        uint16_t    bg_block_bitmap_csum_hi;  // Checksum do bitmap de blocos (bits 16–31).
